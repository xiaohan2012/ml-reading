...
**Tabular foundation models.** Three distinct approaches compete: TabPFN v2 (Nature 2025, ~130M synthetic datasets, alternating row/col attention, best small-table ICL but capped at 10K samples); TabICL (ICML 2025, three-Transformer decoupled architecture, scales to 500K samples, outperforms TabPFNv2 on large data); and CARTE (ICML 2024, graph-per-row pretraining on YAGO knowledge base, no schema matching, best for string-heavy tables). [TabICLv2](pages/qu2026tabiclv2.md) (ICML 2026) extends TabICL with query-aware scalable softmax (QASSMax) to combat attention fading at long context, early target-aware embedding, Muon optimizer, and a richer synthetic prior — becoming the first fully open TFM to surpass closed-source RealTabPFN-2.5 and scaling natively to 1M-row tables. All four challenge gradient-boosted tree dominance on tabular data, but each targets a different regime.
...
**LLM mechanistic interpretability (background).** [Ferrando et al. 2024](pages/ferrando2024primer.md) provides the field's primer: a two-axis taxonomy (**localize** vs **decode**), unified residual-stream notation, and a catalogue of discovered behaviors (head/neuron types, IOI/factual-recall circuits). The page includes a **bridge table** mapping balef's techniques (probing, lens, ablation, self-repair, repr-similarity) onto the wider LLM toolbox, clarifying which methods *don't* transfer (SAEs, circuit discovery, causal interventions with counterfactuals, input attribution) and *why* (tabular ICL lacks clean templates and a 1-D token sequence).

**TFM mechanistic interpretability.** A cluster of contemporaneous 2026 studies open the TFM black box from complementary angles. [Balef et al. 2026](pages/balef2026onelayer.md) study *inference dynamics across depth* (TabPFN v1/v2/v2.5, TabICL, LimiX-2M/16M): TFM inference is *iterative* — each layer incrementally enriches features and widens the [separation gap](pages/balef2026onelayer.md) between classes; early layers are uniquely critical (input-encoder extension), while middle/late layers are largely redundant and self-repairing. They introduce the [tabular logit lens](pages/tabular-logit-lens.md) (per-layer decoders pretrained on TabICL priors), a four-stage [TFM inference taxonomy](pages/tfm-inference-stages.md), and a proof-of-concept [looped nanoTabPFN](pages/looped-transformer-tfm.md) — one layer applied 6× matches a 6-layer model at ~20% parameters. [Biloš et al. 2026](pages/bilos2026mechanistic.md) instead pin down *the readout mechanism* (TabPFNv2/TabICLv2/Mitra): benchmark-equivalent models compute differently — TabPFN/Mitra read out via an attention-weighted **vote** over context labels at L9, TabICL via a nearest-class-**prototype**, and transplanting one readout onto another backbone costs 33–40pp (readout and backbone are jointly designed). They also show permutation invariance is nearly free to install (one-line edits + One-vs-All), that [representation collapse](pages/qu2025tabicl.md) is architectural rather than fundamental (Mitra avoids it natively), and that mechanism-grounded attacks (hub poison, rank warp) reproduce readout-specific failures. Both agree much of each backbone is redundant depth. Narrowest and most causal, [Gupta et al. 2026](pages/gupta2026tabpfnheads.md) (ICML workshop, cited by Biloš as concurrent) probe TabPFN-2.5's feature-wise attention *heads* on two synthetic tasks: one dominant head (2–5× ablation effect, peak layer shifting with task) plus two symmetric late-layer heads, a demonstration that attention selectivity ≠ causal necessity, and a negative result — contrastive activation steering fails to transfer across samples because pure-ICL TFMs lack the function-vector heads that make steering tractable in LLMs.
