...
- [tgat](tgat.md): DyGFormer uses the same Bochner time encoding; unlike TGAT, replaces graph attention over 2-hop neighborhoods with Transformer over long first-hop sequences.
- [tgn](tgn.md): DyGFormer eliminates TGN's memory module (and its RNN gradient issues) by relying on long first-hop history + Transformer; patching handles the long-context problem memory was solving.
- [tgm-temporal-graph-modelling](tgm-temporal-graph-modelling.md): DyGLib is a complementary standardization effort; TGM unifies CTDG+DTDG, DyGLib focuses on CTDG reproducibility and evaluation rigor.
