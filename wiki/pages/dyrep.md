...
- [tgn](tgn.md): TGN explicitly subsumes DyRep as a special case (GRU memory + graph attention in message function); DyRep's two-process distinction maps to edge feature conditioning.
- [tgat](tgat.md): TGAT (2020) replaces DyRep's recurrent update with pure Bochner time encoding + attention, removing the RNN dependency.
- [dygformer](dygformer.md): DyGFormer (2023) eliminates both RNN and multi-hop aggregation; DyRep's recurrent update is replaced by long first-hop Transformer sequences.
