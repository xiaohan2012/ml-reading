...
- [gcn](gcn.md): GAT directly replaces GCN's isotropic degree normalization with feature-based attention; same O(|E|) complexity.
- [graphsage](graphsage.md): both handle inductive learning; GAT uses the full neighborhood while GraphSAGE samples a fixed-size subset.
- [gin](gin.md): GIN's theory shows attention-weighted mean (GAT) is not injective — GAT improves capacity over GCN but is still < WL in expressive power.
- [hgt](hgt.md): HGT extends GAT-style attention to heterogeneous graphs via type-specific key/query matrices per meta-relation triplet.
