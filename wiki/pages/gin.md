...
- [gcn](gcn.md): GIN formally proves GCN's mean aggregation is not injective on multisets, placing GCN strictly below WL power.
- [graphsage](graphsage.md): same analysis applies to GraphSAGE's mean aggregator; max-pool variant is weaker still.
- [gat](gat.md): attention-weighted mean is also not injective — mentioned in GIN paper as a direction for future analysis.
- [relgnn](relgnn.md): RelGNN's schema-aware composite message passing (bridge/hub routes + FUSE) can be seen as moving toward injective aggregation within the RDL schema topology.
