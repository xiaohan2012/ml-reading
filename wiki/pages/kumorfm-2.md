...
- Supersedes [kumorfm-1](kumorfm-1.md) — v2 adds hierarchical attention, task-conditioned processing, billion-scale support, and fine-tuning
- Outperforms [relgnn](relgnn.md) (78.06) on RelBench v1 without supervised training — first RFM to do so
- Shares ICL paradigm with [relational-transformer](relational-transformer.md) (RT) but differs: KumoRFM uses row-level tokens + FK graph attention vs. RT's cell-level tokens + Relational Attention masks
- Contrasts with [griffin](griffin.md) which also pretrained on relational data but uses meta-learning rather than ICL
