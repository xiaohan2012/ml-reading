...
- [gin](gin.md): GIN's proof that sum aggregation = 1-WL completeness is stated in MPNN language; sum readout in MPNN is what GIN formally justifies
- [gat](gat.md): GAT's attention-weighted sum is a special case of M_t with learned scalar weights
- [gcn](gcn.md): GCN's normalized mean aggregation is a special case with `M_t(h_v, h_w) = c_vw · h_w` and `U_t = sigma(W · m_v)`
- [graphsage](graphsage.md): GraphSAGE's LSTM/mean/max aggregators are special cases of M_t + U_t choices
- [graphormer](graphormer.md): Graphormer explicitly shows it can simulate GCN/GIN/GraphSAGE as special cases of its attention — itself built on the MPNN intuition
