...
sources: [relational-deep-learning-position, relational-entity-graph, training-table, tgat, tgn, dyrep, relgnn]
updated: 2026-04-29
---
...
A relational database is a collection of tables linked by **primary key (PK) / foreign key (FK) relationships**. The [relational-deep-learning-position](relational-deep-learning-position.md) paper defines a mechanical, schema-driven conversion:
...
The [training-table](training-table.md) abstraction handles this via the `seed_time` field: when computing the embedding for entity $v$ at seed time $t_v$, **only neighbors $w$ with $\tau(w) \leq t_v$ are included in the computation graph**. This is Algorithm 1 in [relational-deep-learning-position](relational-deep-learning-position.md) — the *time-consistent computation graph*.
...
[relgnn](relgnn.md) further characterizes the specific topological patterns that emerge from PK-FK graphs: **bridge nodes** (exactly 2 FK relationships, appear on paths between entity types) and **hub nodes** (3+ FK relationships, aggregate multiple entity types). Its FUSE operation is designed around these patterns.
...
- [relgnn](relgnn.md) — exploits bridge/hub topology specific to PK-FK graphs
