...
- [qu2025tabicl](qu2025tabicl.md): TabICL's 3-Transformer pipeline (TF_col → TF_row → TF_icl) directly builds on SAINT's insight of combining within-row and between-row attention. TF_col ≈ SAINT's MSA; TF_row ≈ SAINT's MISA. TabICL adds the ICL Transformer and scales to 500K.
- [hollmann2023tabpfnv1](hollmann2023tabpfnv1.md): TabPFN takes a different approach — PFN pretraining eliminates per-dataset training; SAINT requires supervised training. For the same scope, TabPFN is faster; SAINT can be fine-tuned on labeled data.
- [hollmann2025tabpfnv2](hollmann2025tabpfnv2.md): TabPFN v2's alternating row/column attention is architecturally similar to SAINT's MSA+MISA stages.
- [kim2024carte](kim2024carte.md): CARTE uses graph-per-row encoding with YAGO pretraining; SAINT uses row-and-column attention with contrastive pretraining. Both improve on flat tabular baselines but for different data regimes.
