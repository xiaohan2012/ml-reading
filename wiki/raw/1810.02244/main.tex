...
\newcommand{\trans}{^T}
\renewcommand{\vec}[1]{\mathbf{#1}} %TODO: avoid renewcommand!
...
\subsection{Datasets }
To compare our $k$-GNN architectures to kernel approaches we use well-established benchmark datasets from the graph kernel literature~\cite{KKMMN2016}. The nodes of each graph in these dataset is annotated with (discrete) labels or no labels. %See the supplementary material for statistics and properties.
...
\begin{proposition}[Proposition 3 in the main paper]
	...
	\begin{equation*}\label{refine}
		c^{(t)}_{\text{s},k,l} \sqsubseteq f^{(t)}_{k}\,. 
		% c^i_{\text{$k$-GNN}(\mathbf{W^i}, F_l^{(t)})}
	\end{equation*}
	%for all choices of $\mathbf{W}^{(t)}$, $f^{(0)}_k$ consistent with $l$, and $t \geq 0$.
\end{proposition}
...
%\section{Details of the \textsc{Qm9} dataset}

\begin{table*}
	\caption{%
	Details of the  \textsc{Qm9} dataset. $\dagger$: Including atomic energy/enthalpy.
	}%
