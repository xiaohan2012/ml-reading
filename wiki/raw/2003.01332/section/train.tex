...
% During \sampling, we can have multiple event nodes linked to one plain node, such as multiple papers published at WWW at different times. In this case, we treat the same node with different timestamps distinctly, which means that in Algorithm~\ref{alg:budget} line~\ref{line:check} we also use the associated timestamp as a judgment indicator. 
% In this way, WWW@1974 and WWW@2019 can both occur in the sampled subgraph, linking to the same neighborhoods. With relative temporal encoding, the \short\ model should learn to attend differently for these two WWW nodes towards all the papers published on it in different years. 
...
%During this process, we inductively assign timestamps from target nodes to the plain source nodes, if the source nodes don’t have fixed timestamps themselves (e.g., papers assign publication dates to venues). 
