...
%%but the entire AutoML pipeline, that slashes the required time for training and hyperparameter tuning to \textit{less than a second} and simplifies the application to a neural network (NN) forward-pass, a stable, differentiable and highly portable standard procedure.
%%
%%%We propose a radical change, not only to tabular classifiers but the entire AutoML pipeline, that slashes the required time for training and hyperparameter tuning to \textit{less than a second} and simplifies the application to a neural network (NN) forward-pass, a stable, differentiable and highly portable standard procedure. % that could even be run on handheld devices.
